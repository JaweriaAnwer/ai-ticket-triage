...
export function AnimatedHorizontalBarChart({
  ...
  // `as const` prevents TypeScript from treating this as number[]
  ...
  return (
    <div className="w-full h-full flex flex-col justify-center py-4 relative">
      ...
      <div className="flex flex-col gap-4 w-full h-full justify-center">
        {data.map((item, index) => {
          // Middle item appears first, then items move outward
          ...
          // Use item's own color if provided,
          // otherwise use the default color list
          const color =
            item.color ||
...
function HorizontalBar({
  ...
  // Convert seconds to milliseconds for the counter
